...
public sealed class HttpStatusCodeCustomExceptionMiddleware
{
    ...
    {
        _next = next;
        _logger = loggerFactory?.CreateLogger<HttpStatusCodeCustomExceptionMiddleware>() ?? throw new ArgumentNullException(nameof(loggerFactory));
    }
    ...
    private async Task HandleExceptionAsync(HttpContext context, HttpStatusCodeException exception)
    {
        ...
        await context.Response.WriteAsJsonAsync(new HttpStatusCodeErrorDetails((int)exception.StatusCode, exception.Message).Serialize());
    }
}
